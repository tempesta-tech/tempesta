...
static inline void
add_welford_fp_64(struct welford_state_64 *s, long x)
{
	int64_t x_fp;
	int64_t delta;
	int64_t delta2;
...
static inline void
replace_welford_fp_64(struct client *c,
		   struct welford_state_64 *s,
...
static inline void
adjust_welford_fp_64(struct client *c, struct welford_state_64 *s, long new_max)
{
	/*
	 * Existing client update - remove old value, add new value
	 */
	replace_welford_fp_64(c, s, new_max);
}
...
static void
adjust_sum_sumsq_64(struct client *c, struct sum_sumsq_state_64 *s, unsigned long new_max)
{
...
static long double
variance_sum_sumsq_64(struct sum_sumsq_state_64 *s)
{
	long double mean;
...
static long double
variance_welford_64(struct welford_state_64 *s)
{
	if (!s->n)
		return 0;
