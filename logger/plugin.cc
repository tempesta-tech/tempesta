...
Plugin::Plugin(std::string_view plugin_path, const ClickHouseConfig& config)
{
...
EventProcessorPtr Plugin::create_processor(TfwLoggerProcessorContext context) {
	if (!api_ || !api_->create_processor)
		throw tus::Except("Plugin {} not properly loaded", name_);

	void* raw_processor = api_->create_processor(context);
	if (!raw_processor)
