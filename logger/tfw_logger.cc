...
/**
 * Debug hexdump for troubleshooting binary data.
 */
#ifdef DEBUG
static void
...
/**
 * Log error with optional spdlog support.
 */
void 
log_error(const std::string& msg, bool to_spdlog, bool unhandled)
{
	std::string full_msg = unhandled ? "Unhandled error: " + msg : msg;
	
	if (to_spdlog)
		spdlog::error(full_msg);
	else
		std::cerr << full_msg << std::endl;
}

/**
 * Create ClickHouse block schema for access log.
 */
clickhouse::Block
make_block()
{
...
/**
 * Read and process access log event from binary data.
 */
int
...
/**
 * Callback function called when data is available in mmap buffer.
 */
void
...
/**
 * Worker thread function for processing mmap buffer from one CPU.
 */
void 
