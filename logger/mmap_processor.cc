...
MmapProcessor::MmapProcessor(std::shared_ptr<TfwClickhouse> db,
	...
	: EventProcessor(std::move(db))
	, device_fd_(device_fd)
	, cpu_id_(context.cpu_id)
	, stop_flag_(context.stop_flag)
{
	plugin_log_debug("Creating MmapProcessor with device: {}", device_fd_);

	unsigned int area_size;

	size_ = get_buffer_size(fd);

	area_size = TFW_MMAP_BUFFER_FULL_SIZE(size_);

	buffer_ = (TfwMmapBuffer *)mmap(nullptr, area_size,
					PROT_READ|PROT_WRITE,
					MAP_SHARED, fd, area_size * cpu_id_);
...
Error<bool> MmapProcessor::consume_event()
{
	uint64_t head, tail;
...
void MmapProcessor::make_background_work()
{
	flush(true);
}

bool MmapProcessor::flush(bool force) noexcept
{
	assert(db_);

	if (force)
		return db_->commit(TfwClickhouse::FROCE);
