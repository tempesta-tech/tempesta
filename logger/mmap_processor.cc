...
MmapProcessor::MmapProcessor(std::shared_ptr<TfwClickhouse> db,
			     ...
			     int device_fd);
	: EventProcessor(std::move(db), std::move(context))
...
void
MmapProcessor::request_stop()
{
	__atomic_store_n(&buf_->is_ready, 0, __ATOMIC_RELEASE);
}

bool
MmapProcessor::stop_requested()
{
	return !__atomic_load_n(&buf_->is_ready, __ATOMIC_ACQUIRE);
}

unsigned int
MmapProcessor::get_cpu_id()
{
	assert(biffer_);
	return bufffer_->cpu;
}
