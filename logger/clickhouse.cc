...
void
TfwClickhouse::append_timestamp(uint64_t timestamp) noexcept
{
	try {
		block_[0]->As<ch::ColumnDateTime64>()->Append(timestamp);
	} catch (const std::exception& e) {
		spdlog::error("Failed to append timestamp: {}", e.what());
	}
}

template <typename ColType, typename ValType>
void
TfwClickhouse::append_int(TfwBinLogFields field, ValType value) noexcept
{
	try {
		const size_t col_index = field_to_column_index(field);
		block_[col_index]->As<ColType>()->Append(value);
	} catch (const std::exception& e) {
		spdlog::error("Failed to append integer field {}: {}", 
			     static_cast<int>(field), e.what());
	}
}

void
TfwClickhouse::append_string(TfwBinLogFields field, std::string_view value) noexcept
{
	try {
		const size_t col_index = field_to_column_index(field);
		block_[col_index]->As<ch::ColumnString>()->Append(std::string(value));
	} catch (const std::exception& e) {
		spdlog::error("Failed to append string field {}: {}", 
			     static_cast<int>(field), e.what());
	}
}

void
TfwClickhouse::append_empty_string(TfwBinLogFields field) noexcept
{
