...
static void ssl_write_hostname_ext(ttls_ssl_context *ssl,
		unsigned char *buf,
		size_t *olen)
{
	unsigned char *p = buf;
	...
	*olen = 0;
	...
	TTLS_SSL_DEBUG_MSG(3, ("client hello, adding server name extension: %s",
				   ssl->hostname));
		...
		size_t *olen)
{
	...
	*olen = 0;
	...
	if (ssl->conf->max_minor_ver != TTLS_SSL_MINOR_VERSION_3)
		return;

	TTLS_SSL_DEBUG_MSG(3, ("client hello, adding signature_algorithms extension"));
	...
	{
#if defined(TTLS_ECDSA_C)
		sig_alg_list[sig_alg_len++] = ttls_ssl_hash_from_md_alg(*md);
		sig_alg_list[sig_alg_len++] = TTLS_SSL_SIG_ECDSA;
#endif
		sig_alg_list[sig_alg_len++] = ttls_ssl_hash_from_md_alg(*md);
		sig_alg_list[sig_alg_len++] = TTLS_SSL_SIG_RSA;
	}
...
static void ssl_write_supported_elliptic_curves_ext(ttls_ssl_context *ssl,
		 unsigned char *buf,
		 size_t *olen)
{
	unsigned char *p = buf;
	...
	*olen = 0;
	...
	TTLS_SSL_DEBUG_MSG(3, ("client hello, adding supported_elliptic_curves extension"));
	...
	for (grp_id = ssl->conf->curve_list; *grp_id != TTLS_ECP_DP_NONE; grp_id++)
	{
		...
		if (info == NULL)
		{
			TTLS_SSL_DEBUG_MSG(1, ("invalid curve in ssl configuration"));
			return;
		}
		...
		size_t *olen)
{
	...
	*olen = 0;
	...
	TTLS_SSL_DEBUG_MSG(3, ("client hello, adding supported_point_formats extension"));
		...
		size_t *olen)
{
	...
	*olen = 0;
	...
	TTLS_SSL_DEBUG_MSG(3, ("client hello, adding ecjpake_kkpp extension"));
	...
	*p++ = (unsigned char)((TTLS_TLS_EXT_ECJPAKE_KKPP	 ) & 0xFF);
	...
	{
		TTLS_SSL_DEBUG_MSG(3, ("generating new ecjpake parameters"));
		...
		if (ssl->handshake->ecjpake_cache == NULL)
		{
			TTLS_SSL_DEBUG_MSG(1, ("allocation failed"));
			return;
		}
	...
	{
		TTLS_SSL_DEBUG_MSG(3, ("re-using cached ecjpake parameters"));
		...
		size_t *olen)
{
	...
	*olen = 0;
	...
	if (ssl->conf->mfl_code == TTLS_SSL_MAX_FRAG_LEN_NONE) {
		return;
	}

	TTLS_SSL_DEBUG_MSG(3, ("client hello, adding max_fragment_length extension"));
		...
		unsigned char *buf, size_t *olen)
{
	...
	*olen = 0;
	...
	if (ssl->conf->extended_ms == TTLS_SSL_EXTENDED_MS_DISABLED ||
		ssl->conf->max_minor_ver == TTLS_SSL_MINOR_VERSION_0)
	{
		return;
	}

	TTLS_SSL_DEBUG_MSG(3, ("client hello, adding extended_master_secret "
						"extension"));
		...
		unsigned char *buf, size_t *olen)
{
	...
	*olen = 0;
	...
	if (ssl->conf->session_tickets == TTLS_SSL_SESSION_TICKETS_DISABLED)
	{
		return;
	}

	TTLS_SSL_DEBUG_MSG(3, ("client hello, adding session ticket extension"));
	...
	*olen = 4;
	...
	TTLS_SSL_DEBUG_MSG(3, ("sending session ticket of length %d", tlen));
		...
		unsigned char *buf, size_t *olen)
{
	...
	*olen = 0;
	...
	TTLS_SSL_DEBUG_MSG(3, ("client hello, adding alpn extension"));
 ...
 */
static int ssl_generate_random(ttls_ssl_context *ssl)
{
	int ret;
	...
	*p++ = (unsigned char)(t	  );
	...
	TTLS_SSL_DEBUG_MSG(3, ("client hello, current time: %lu", t));
...
static int ssl_write_client_hello(ttls_ssl_context *ssl)
{
	int ret;
	...
	TTLS_SSL_DEBUG_MSG(2, ("=> write client hello"));
	...
	{
		TTLS_SSL_DEBUG_MSG(1, ("configured max major version is invalid, "
				"consider using ttls_ssl_config_defaults()"));
	...
	ttls_ssl_write_version(ssl->conf->max_major_ver, ssl->conf->max_minor_ver,
					   ssl->conf->transport, p);
	p += 2;

	TTLS_SSL_DEBUG_MSG(3, ("client hello, max version: [%d:%d]",
				   buf[4], buf[5]));

	if ((ret = ssl_generate_random(ssl)) != 0)
	{
		TTLS_SSL_DEBUG_RET(1, "ssl_generate_random", ret);
		return ret;
	}

	memcpy(p, ssl->handshake->randbytes, 32);
	TTLS_SSL_DEBUG_BUF(3, "client hello, random bytes", p, 32);
	p += 32;
	...
	*p++ = (unsigned char) n;
	...
	TTLS_SSL_DEBUG_MSG(3, ("client hello, session id len.: %d", n));
	TTLS_SSL_DEBUG_BUF(3,   "client hello, session id", buf + 39, n);
	...
	if (ssl->conf->transport == TTLS_SSL_TRANSPORT_DATAGRAM)
	{
		...
		{
			TTLS_SSL_DEBUG_MSG(3, ("no verify cookie to send"));
			*p++ = 0;
		}
		else
		{
	...
	{
		ciphersuite_info = ttls_ssl_ciphersuite_from_id(ciphersuites[i]);
		...
		TTLS_SSL_DEBUG_MSG(3, ("client hello, add ciphersuite: %04x",
							ciphersuites[i]));
	...
	TTLS_SSL_DEBUG_MSG(3, ("client hello, got %d ciphersuites (excluding SCSVs)", n));

	/*
	 * Add TLS_EMPTY_RENEGOTIATION_INFO_SCSV
	 */
	TTLS_SSL_DEBUG_MSG(3, ("adding EMPTY_RENEGOTIATION_INFO_SCSV"));
	*p++ = (unsigned char)(TTLS_SSL_EMPTY_RENEGOTIATION_INFO >> 8);
	*p++ = (unsigned char)(TTLS_SSL_EMPTY_RENEGOTIATION_INFO	 );
	n++;

	/* Some versions of OpenSSL don't handle it correctly if not at end */
#if defined(TTLS_SSL_FALLBACK_SCSV)
	if (ssl->conf->fallback == TTLS_SSL_IS_FALLBACK)
	{
		TTLS_SSL_DEBUG_MSG(3, ("adding FALLBACK_SCSV"));
		*p++ = (unsigned char)(TTLS_SSL_FALLBACK_SCSV_VALUE >> 8);
		*p++ = (unsigned char)(TTLS_SSL_FALLBACK_SCSV_VALUE	 );
		n++;
	}
#endif
	...
	*q++ = (unsigned char)(n << 1);
	...
	if (offer_compress)
	{
		TTLS_SSL_DEBUG_MSG(3, ("client hello, compress len.: %d", 2));
		TTLS_SSL_DEBUG_MSG(3, ("client hello, compress alg.: %d %d",
			TTLS_SSL_COMPRESS_DEFLATE, TTLS_SSL_COMPRESS_NULL));

		*p++ = 2;
		*p++ = TTLS_SSL_COMPRESS_DEFLATE;
		*p++ = TTLS_SSL_COMPRESS_NULL;
	}
	else
	{
		TTLS_SSL_DEBUG_MSG(3, ("client hello, compress len.: %d", 1));
		TTLS_SSL_DEBUG_MSG(3, ("client hello, compress alg.: %d",
						TTLS_SSL_COMPRESS_NULL));
	...
	TTLS_SSL_DEBUG_MSG(3, ("client hello, total extension length: %d",
				   ext_len));
...
#if defined(TTLS_ECDH_C) || defined(TTLS_ECDSA_C) || \
	defined(TTLS_KEY_EXCHANGE_ECJPAKE_ENABLED)
static int ssl_parse_supported_point_formats_ext(ttls_ssl_context *ssl,
		const unsigned char *buf,
		size_t len)
{
	size_t list_size;
 ...
 */
#if defined(TTLS_SSL_PROTO_DTLS)
static int ssl_parse_hello_verify_request(ttls_ssl_context *ssl)
{
	const unsigned char *p = ssl->in_msg + ttls_ssl_hs_hdr_len(ssl);
	int major_ver, minor_ver;
	unsigned char cookie_len;

	TTLS_SSL_DEBUG_MSG(2, ("=> parse hello verify request"));
	 ...
	 */
	if (major_ver < TTLS_SSL_MAJOR_VERSION_3 ||
		minor_ver < TTLS_SSL_MINOR_VERSION_2 ||
	...
	cookie_len = *p++;
	TTLS_SSL_DEBUG_BUF(3, "cookie", p, cookie_len);
	...
	if (ssl->handshake->verify_cookie  == NULL)
	{
		TTLS_SSL_DEBUG_MSG(1, ("alloc failed (%d bytes)", cookie_len));
...
static int ssl_parse_server_hello(ttls_ssl_context *ssl)
{
	int ret, i;
	size_t n;
	size_t ext_len;
	...
	TTLS_SSL_DEBUG_MSG(2, ("=> parse server hello"));
	...
	if (ssl->conf->transport == TTLS_SSL_TRANSPORT_DATAGRAM)
	{
		if (buf[0] == TTLS_SSL_HS_HELLO_VERIFY_REQUEST)
		{
			TTLS_SSL_DEBUG_MSG(2, ("received hello verify request"));
			TTLS_SSL_DEBUG_MSG(2, ("<= parse server hello"));
	...
	if (ssl->in_hslen < 38 + ttls_ssl_hs_hdr_len(ssl) ||
	...
	TTLS_SSL_DEBUG_BUF(3, "server hello, version", buf + 0, 2);
	...
	{
		TTLS_SSL_DEBUG_MSG(1, ("server version out of bounds - "
	...
	TTLS_SSL_DEBUG_MSG(3, ("server hello, current time: %lu",
			   ((uint32_t) buf[2] << 24) |
			   ((uint32_t) buf[3] << 16) |
			   ((uint32_t) buf[4] <<  8) |
			   ((uint32_t) buf[5]	  )));
	...
	TTLS_SSL_DEBUG_BUF(3,   "server hello, random bytes", buf + 2, 32);
	...
	if (ssl->in_hslen > ttls_ssl_hs_hdr_len(ssl) + 39 + n)
	{
		ext_len = ((buf[38 + n] <<  8)
				  | (buf[39 + n]	  ));
	...
	else if (ssl->in_hslen == ttls_ssl_hs_hdr_len(ssl) + 38 + n)
	{
		ext_len = 0;
	}
	else
	 ...
	 */
	ssl->transform_negotiate->ciphersuite_info = ttls_ssl_ciphersuite_from_id(i);
	...
	ttls_ssl_optimize_checksum(ssl, ssl->transform_negotiate->ciphersuite_info);

	TTLS_SSL_DEBUG_MSG(3, ("server hello, session id len.: %d", n));
	TTLS_SSL_DEBUG_BUF(3,   "server hello, session id", buf + 35, n);
	...
	TTLS_SSL_DEBUG_MSG(3, ("server hello, chosen ciphersuite: %04x", i));
	...
	TTLS_SSL_DEBUG_MSG(3, ("server hello, chosen ciphersuite: %s", suite_info->name));
	...
	TTLS_SSL_DEBUG_MSG(2, ("server hello, total extension length: %d", ext_len));
	...
	while (ext_len)
	{
		...
		case TTLS_TLS_EXT_MAX_FRAGMENT_LENGTH:
			TTLS_SSL_DEBUG_MSG(3, ("found max_fragment_length extension"));
		...
		case TTLS_TLS_EXT_EXTENDED_MASTER_SECRET:
			TTLS_SSL_DEBUG_MSG(3, ("found extended_master_secret extension"));
		...
		case TTLS_TLS_EXT_SESSION_TICKET:
			TTLS_SSL_DEBUG_MSG(3, ("found session_ticket extension"));
	...
	defined(TTLS_KEY_EXCHANGE_ECJPAKE_ENABLED)
		case TTLS_TLS_EXT_SUPPORTED_POINT_FORMATS:
			TTLS_SSL_DEBUG_MSG(3, ("found supported_point_formats extension"));
		...
		case TTLS_TLS_EXT_ECJPAKE_KKPP:
			TTLS_SSL_DEBUG_MSG(3, ("found ecjpake_kkpp extension"));
...
static int ssl_parse_server_dh_params(ttls_ssl_context *ssl, unsigned char **p,
		unsigned char *end)
{
	...
	{
		TTLS_SSL_DEBUG_RET(2, ("ttls_dhm_read_params"), ret);
		return ret;
	}

	if (ssl->handshake->dhm_ctx.len * 8 < ssl->conf->dhm_min_bitlen)
	{
		TTLS_SSL_DEBUG_MSG(1, ("DHM prime too short: %d < %d",
...
static int ssl_check_server_ecdh_params(const ttls_ssl_context *ssl)
{
	...
	TTLS_SSL_DEBUG_MSG(2, ("ECDH curve: %s", curve_info->name));

	if (ttls_ssl_check_curve(ssl, ssl->handshake->ecdh_ctx.grp.id) != 0)
		return(-1);
	...
	defined(TTLS_KEY_EXCHANGE_ECDHE_ECDSA_ENABLED) || \
	defined(TTLS_KEY_EXCHANGE_ECDHE_PSK_ENABLED)
static int ssl_parse_server_ecdh_params(ttls_ssl_context *ssl,
		unsigned char **p,
		unsigned char *end)
{
	int ret = TTLS_ERR_SSL_FEATURE_UNAVAILABLE;
	...
	{
		TTLS_SSL_DEBUG_RET(1, ("ttls_ecdh_read_params"), ret);
		return ret;
	}

	if (ssl_check_server_ecdh_params(ssl) != 0)
	{
		TTLS_SSL_DEBUG_MSG(1, ("bad server key exchange message (ECDHE curve)"));
		return(TTLS_ERR_SSL_BAD_HS_SERVER_KEY_EXCHANGE);
	}
...
static int ssl_parse_server_psk_hint(ttls_ssl_context *ssl,
		  unsigned char **p,
		  unsigned char *end)
{
	int ret = TTLS_ERR_SSL_FEATURE_UNAVAILABLE;
 ...
 */
static int ssl_write_encrypted_pms(ttls_ssl_context *ssl,
		size_t offset, size_t *olen,
		size_t pms_offset)
{
	int ret;
	size_t len_bytes = ssl->minor_ver == TTLS_SSL_MINOR_VERSION_0 ? 0 : 2;
	unsigned char *p = ssl->handshake->premaster + pms_offset;

	if (offset + len_bytes > TTLS_SSL_MAX_CONTENT_LEN)
	{
		TTLS_SSL_DEBUG_MSG(1, ("buffer too small for encrypted pms"));
	...
	{
		TTLS_SSL_DEBUG_RET(1, "f_rng", ret);
		return ret;
	}
	...
	if (! ttls_pk_can_do(&ssl->session_negotiate->peer_cert->pk,
				TTLS_PK_RSA))
	{
		TTLS_SSL_DEBUG_MSG(1, ("certificate key type mismatch"));
	...
	if ((ret = ttls_pk_encrypt(&ssl->session_negotiate->peer_cert->pk,
			...
			TTLS_SSL_MAX_CONTENT_LEN - offset - len_bytes,
			ssl->conf->f_rng, ssl->conf->p_rng)) != 0)
	{
		TTLS_SSL_DEBUG_RET(1, "ttls_rsa_pkcs1_encrypt", ret);
		return ret;
	}
	...
	defined(TTLS_KEY_EXCHANGE_ECDHE_ECDSA_ENABLED)
static int ssl_parse_signature_algorithm(ttls_ssl_context *ssl,
		  unsigned char **p,
		  unsigned char *end,
		  ...
		  ttls_pk_type_t *pk_alg)
{
	...
	*pk_alg = TTLS_PK_NONE;
	 ...
	 * Get hash algorithm
	 */
	if ((*md_alg = ttls_ssl_md_alg_from_hash((*p)[0])) == TTLS_MD_NONE)
	{
		TTLS_SSL_DEBUG_MSG(1, ("Server used unsupported "
					"HashAlgorithm %d", *(p)[0]));
		return(TTLS_ERR_SSL_BAD_HS_SERVER_KEY_EXCHANGE);
	}
	 ...
	 */
	if ((*pk_alg = ttls_ssl_pk_alg_from_sig((*p)[1])) == TTLS_PK_NONE)
	{
		TTLS_SSL_DEBUG_MSG(1, ("server used unsupported "
	 ...
	 */
	if (ttls_ssl_check_sig_hash(ssl, *md_alg) != 0)
	{
		TTLS_SSL_DEBUG_MSG(1, ("server used HashAlgorithm %d that was not offered",
									*(p)[0]));
		return(TTLS_ERR_SSL_BAD_HS_SERVER_KEY_EXCHANGE);
	}

	TTLS_SSL_DEBUG_MSG(2, ("Server used SignatureAlgorithm %d", (*p)[1]));
	TTLS_SSL_DEBUG_MSG(2, ("Server used HashAlgorithm %d", (*p)[0]));
	*p += 2;
...
static int ssl_get_ecdh_params_from_cert(ttls_ssl_context *ssl)
{
	...
	if (ssl_check_server_ecdh_params(ssl) != 0)
	{
		TTLS_SSL_DEBUG_MSG(1, ("bad server certificate (ECDH curve)"));
...
static int ssl_parse_server_key_exchange(ttls_ssl_context *ssl)
{
	int ret;
	const ttls_ssl_ciphersuite_t *ciphersuite_info =
	 ...
	 */
	if (ssl->in_msg[0] != TTLS_SSL_HS_SERVER_KEY_EXCHANGE)
	{
	...
	TTLS_SSL_DEBUG_BUF(3,   "server key exchange", p, end - p);
	...
	if (ttls_ssl_ciphersuite_uses_server_signature(ciphersuite_info))
	{
		...
		unsigned char *params = ssl->in_msg + ttls_ssl_hs_hdr_len(ssl);
		...
		TTLS_SSL_DEBUG_BUF(3, "signature", p, sig_len);
		...
		if (md_alg != TTLS_MD_NONE)
		{
			...
			hashlen = 0;
			ret = ttls_ssl_get_key_exchange_md_tls1_2(ssl, hash, params,
								  params_len, md_alg);
			if (ret != 0)
				return ret;
		}
...
static int ssl_parse_certificate_request(ttls_ssl_context *ssl)
{
	const ttls_ssl_ciphersuite_t *ciphersuite_info =
		ssl->transform_negotiate->ciphersuite_info;

	TTLS_SSL_DEBUG_MSG(2, ("=> parse certificate request"));

	if (! ttls_ssl_ciphersuite_cert_req_allowed(ciphersuite_info))
	{
		TTLS_SSL_DEBUG_MSG(2, ("<= skip parse certificate request"));
		ssl->state++;
		return 0;
	}
...
static int ssl_parse_certificate_request(ttls_ssl_context *ssl)
{
	...
	ssl->state++;
	ssl->client_auth = (ssl->in_msg[0] == TTLS_SSL_HS_CERTIFICATE_REQUEST);

	TTLS_SSL_DEBUG_MSG(3, ("got %s certificate request",
	...
	/* certificate_types */
	cert_type_len = buf[ttls_ssl_hs_hdr_len(ssl)];
	n = cert_type_len;
	...
	if (ssl->minor_ver == TTLS_SSL_MINOR_VERSION_3)
	{
		...
		{
			TTLS_SSL_DEBUG_MSG(3, ("Supported Signature Algorithm found: %d"
...
static int ssl_parse_server_hello_done(ttls_ssl_context *ssl)
{
	int ret;
	...
	if (ssl->in_hslen  != ttls_ssl_hs_hdr_len(ssl) ||
...
static int ssl_write_client_key_exchange(ttls_ssl_context *ssl)
{
	int ret;
	size_t i, n;
	const ttls_ssl_ciphersuite_t *ciphersuite_info =
	...
	TTLS_SSL_DEBUG_MSG(2, ("=> write client key exchange"));
	...
	if (ciphersuite_info->key_exchange == TTLS_KEY_EXCHANGE_DHE_RSA)
	{
		...
		if (ret != 0)
		{
			TTLS_SSL_DEBUG_RET(1, "ttls_dhm_make_public", ret);
			return ret;
		}
		...
		ciphersuite_info->key_exchange == TTLS_KEY_EXCHANGE_ECDH_ECDSA)
	{
		...
		if (ret != 0)
		{
			TTLS_SSL_DEBUG_RET(1, "ttls_ecdh_make_public", ret);
			return ret;
		}
		...
		TTLS_SSL_DEBUG_MPI(3, "ECDH: z", &ssl->handshake->ecdh_ctx.z);
		  ...
		  TTLS_KEY_EXCHANGE_ECDH_ECDSA_ENABLED */
#if defined(TTLS_KEY_EXCHANGE__SOME__PSK_ENABLED)
	if (ttls_ssl_ciphersuite_uses_psk(ciphersuite_info))
	{
		/*
		...
		{
			TTLS_SSL_DEBUG_MSG(1, ("got no private key for PSK"));
		...
		if (i + 2 + n > TTLS_SSL_MAX_CONTENT_LEN)
		{
			TTLS_SSL_DEBUG_MSG(1, ("psk identity too long or "
		...
		if (ciphersuite_info->key_exchange == TTLS_KEY_EXCHANGE_DHE_PSK)
		{
			...
			if (i + 2 + n > TTLS_SSL_MAX_CONTENT_LEN)
			{
				TTLS_SSL_DEBUG_MSG(1, ("psk identity or DHM size too long"
		...
		if (ciphersuite_info->key_exchange == TTLS_KEY_EXCHANGE_ECDHE_PSK)
		{
			...
			ret = ttls_ecdh_make_public(&ssl->handshake->ecdh_ctx, &n,
					&ssl->out_msg[i], TTLS_SSL_MAX_CONTENT_LEN - i,
					ssl->conf->f_rng, ssl->conf->p_rng);
			if (ret != 0)
			{
				TTLS_SSL_DEBUG_RET(1, "ttls_ecdh_make_public", ret);
	...
	if (ciphersuite_info->key_exchange == TTLS_KEY_EXCHANGE_ECJPAKE)
	{
		...
		ret = ttls_ecjpake_write_round_two(&ssl->handshake->ecjpake_ctx,
				ssl->out_msg + i, TTLS_SSL_MAX_CONTENT_LEN - i, &n,
				ssl->conf->f_rng, ssl->conf->p_rng);
		if (ret != 0)
		{
	...
	!defined(TTLS_KEY_EXCHANGE_ECDH_ECDSA_ENABLED)&& \
	!defined(TTLS_KEY_EXCHANGE_ECDHE_ECDSA_ENABLED)
static int ssl_write_certificate_verify(ttls_ssl_context *ssl)
{
	const ttls_ssl_ciphersuite_t *ciphersuite_info =
		ssl->transform_negotiate->ciphersuite_info;
	int ret;

	TTLS_SSL_DEBUG_MSG(2, ("=> write certificate verify"));
	...
	{
		TTLS_SSL_DEBUG_MSG(2, ("<= skip write certificate verify"));
		ssl->state++;
		return 0;
	}
...
static int ssl_write_certificate_verify(ttls_ssl_context *ssl)
{
	...
	if (ssl->client_auth == 0 || ttls_ssl_own_cert(ssl) == NULL)
	{
		TTLS_SSL_DEBUG_MSG(2, ("<= skip write certificate verify"));
		ssl->state++;
		return 0;
	...
	{
		TTLS_SSL_DEBUG_MSG(1, ("got no private key for certificate"));
		return(TTLS_ERR_SSL_PRIVATE_KEY_REQUIRED);
	}
	...
	if (ssl->minor_ver == TTLS_SSL_MINOR_VERSION_3)
	{
			...
			TTLS_MD_SHA384)
		{
			...
			ssl->out_msg[4] = TTLS_SSL_HASH_SHA384;
		}
		else
		{
			md_alg = TTLS_MD_SHA256;
			ssl->out_msg[4] = TTLS_SSL_HASH_SHA256;
		}
		ssl->out_msg[5] = ttls_ssl_sig_from_pk(ttls_ssl_own_key(ssl));
	...
	if ((ret = ttls_pk_sign(ttls_ssl_own_key(ssl), md_alg, hash_start, hashlen,
...
#if defined(TTLS_SSL_SESSION_TICKETS)
static int ssl_parse_new_session_ticket(ttls_ssl_context *ssl)
{
	int ret;
	uint32_t lifetime;
	size_t ticket_len;
	 ...
	 */
	if (ssl->in_msg[0] != TTLS_SSL_HS_NEW_SESSION_TICKET ||
	...
	msg = ssl->in_msg + ttls_ssl_hs_hdr_len(ssl);
	...
	TTLS_SSL_DEBUG_MSG(3, ("ticket length: %d", ticket_len));
	 ...
	 */
	TTLS_SSL_DEBUG_MSG(3, ("ticket in use, discarding session id"));
...
int ttls_ssl_handshake_client_step(ttls_ssl_context *ssl)
{
	...
	TTLS_SSL_DEBUG_MSG(2, ("client state: %d", ssl->state));
	...
	if (ssl->state == TTLS_SSL_SERVER_CHANGE_CIPHER_SPEC &&
		ssl->handshake->new_session_ticket != 0)
	{
		ssl->state = TTLS_SSL_SERVER_NEW_SESSION_TICKET;
	}
#endif
	...
	switch(ssl->state)
	{
		...
		*/
#if defined(TTLS_SSL_SESSION_TICKETS)
	   case TTLS_SSL_SERVER_NEW_SESSION_TICKET:
