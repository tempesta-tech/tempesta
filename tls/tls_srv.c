...
static int ssl_parse_servername_ext(ttls_context *tls,
				 ...
				 size_t len)
{
	...
	TTLS_DEBUG_MSG(3, ("parse ServerName extension"));

	servername_list_size = ((buf[0] << 8) | (buf[1]));
	if (servername_list_size + 2 != len)
	{
		TTLS_DEBUG_MSG(1, ("bad client hello message"));
	...
	while (servername_list_size > 0)
	{
		...
		if (p[0] == TTLS_TLS_EXT_SERVERNAME_HOSTNAME)
		{
			...
			if (r != 0)
			{
				TTLS_DEBUG_RET(1, "ssl_sni_wrapper", r);
				ttls_send_alert_message(tls, TTLS_ALERT_LEVEL_FATAL,
...
static int ssl_parse_renegotiation_info(ttls_context *tls,
					 const unsigned char *buf,
					 size_t len)
{
	if (len != 1 || buf[0] != 0x0)
	...
	tls->secure_renegotiation = TTLS_SECURE_RENEGOTIATION;
 ...
 *
 */
static int ssl_parse_signature_algorithms_ext(ttls_context *tls,
					   const unsigned char *buf,
					   size_t len)
{
	...
	for (p = buf + 2; p < end; p += 2)
	{
		...
		{
			TTLS_DEBUG_MSG(3, ("client hello v3, signature_algorithm ext"
		...
		if (ttls_check_sig_hash(tls, md_cur) == 0)
		{
...
static int ssl_parse_supported_point_formats(ttls_context *tls,
					  const unsigned char *buf,
					  size_t len)
{
	size_t list_size;
...
static int ssl_parse_max_fragment_length_ext(ttls_context *tls,
					  ...
					  size_t len)
{
	if (len != 1 || buf[0] >= TTLS_MAX_FRAG_LEN_INVALID)
	{
		TTLS_DEBUG_MSG(1, ("bad client hello message"));
		ttls_send_alert_message(tls, TTLS_ALERT_LEVEL_FATAL,
						TTLS_ALERT_MSG_ILLEGAL_PARAMETER);
...
static int ssl_parse_encrypt_then_mac_ext(ttls_context *tls,
					  const unsigned char *buf,
					  size_t len)
{
	if (len != 0)
...
static int ssl_parse_extended_ms_ext(ttls_context *tls,
				  const unsigned char *buf,
				  size_t len)
{
	if (len != 0)
	...
	if (tls->conf->extended_ms == TTLS_EXTENDED_MS_ENABLED &&
		tls->minor_ver != TTLS_MINOR_VERSION_0)
	{
		tls->hs->extended_ms = TTLS_EXTENDED_MS_ENABLED;
	}
...
static int ssl_parse_session_ticket_ext(ttls_context *tls,
					 unsigned char *buf,
					 size_t len)
{
	int r;
	...
	TTLS_DEBUG_MSG(3, ("ticket length: %d", len));
					   ...
					   buf, len)) != 0)
	{
		...
		if (r == TTLS_ERR_INVALID_MAC)
			TTLS_DEBUG_MSG(3, ("ticket is not authentic"));
		else if (r == TTLS_ERR_SESSION_TICKET_EXPIRED)
			TTLS_DEBUG_MSG(3, ("ticket is expired"));
		else
			TTLS_DEBUG_RET(1, "ttls_ticket_parse", r);
	...
	memcpy(tls->session_negotiate, &session, sizeof(TlsSess));

	/* Zeroize instead of free as we copied the content */
	bzero_fast(&session, sizeof(TtlsSess));

	TTLS_DEBUG_MSG(3, ("session successfully restored from ticket"));
...
static int ssl_parse_alpn_ext(ttls_context *tls,
 ...
 */
static int ssl_pick_cert(ttls_context *tls,
			  const ttls_ciphersuite_t * ciphersuite_info)
{
	ttls_key_cert *cur, *list, *fallback = NULL;
	ttls_pk_type_t pk_alg =
		ttls_get_ciphersuite_sig_pk_alg(ciphersuite_info);
	uint32_t flags;

	if (tls->hs->sni_key_cert != NULL)
	...
	TTLS_DEBUG_MSG(3, ("ciphersuite requires certificate"));

	if (list == NULL)
	{
		TTLS_DEBUG_MSG(3, ("server has no certificate"));
		return(-1);
	}

	for (cur = list; cur != NULL; cur = cur->next)
	{
		TTLS_DEBUG_CRT(3, "candidate certificate chain, certificate",
						  cur->cert);

		if (! ttls_pk_can_do(cur->key, pk_alg))
		{
			TTLS_DEBUG_MSG(3, ("certificate mismatch: key type"));
			continue;
		}
		 ...
		 */
		if (ttls_check_cert_usage(cur->cert, ciphersuite_info,
						  TTLS_IS_SERVER, &flags) != 0)
		{
			TTLS_DEBUG_MSG(3, ("certificate mismatch: "
		...
		if (pk_alg == TTLS_PK_ECDSA &&
			ssl_check_key_curve(cur->key, tls->hs->curves) != 0)
		{
			TTLS_DEBUG_MSG(3, ("certificate mismatch: elliptic curve"));
			continue;
		}
#endif
			...
			cur->cert->sig_md != TTLS_MD_SHA1)
		{
			if (fallback == NULL)
				fallback = cur;
			TTLS_DEBUG_MSG(3, ("certificate not preferred: "
	...
	{
		tls->hs->key_cert = cur;
		TTLS_DEBUG_CRT(3, "selected certificate chain, certificate",
						  tls->hs->key_cert->cert);
		return 0;
	}
 ...
 */
static int ssl_ciphersuite_match(ttls_context *tls, int suite_id,
				  const ttls_ciphersuite_t **ciphersuite_info)
{
	...
	TTLS_DEBUG_MSG(3, ("trying ciphersuite: %s", suite_info->name));

	if (suite_info->min_minor_ver > tls->minor_ver ||
		suite_info->max_minor_ver < tls->minor_ver)
	{
		TTLS_DEBUG_MSG(3, ("ciphersuite mismatch: version"));
		return 0;
	}
	...
	if (ttls_ciphersuite_uses_ec(suite_info) &&
		(tls->hs->curves == NULL ||
		  tls->hs->curves[0] == NULL))
	{
		TTLS_DEBUG_MSG(3, ("ciphersuite mismatch: "
	...
	if (tls->minor_ver == TTLS_MINOR_VERSION_3)
	{
		...
		if (sig_type != TTLS_PK_NONE &&
			ttls_sig_hash_set_find(&tls->hs->hash_algs, sig_type) == TTLS_MD_NONE)
		{
			TTLS_DEBUG_MSG(3, ("ciphersuite mismatch: no suitable hash algorithm "
 ...
 */
static int
ttls_parse_client_hello(TtlsCtx *tls)
{
...
read_record_header:
	...
	// return T_POSTPONE if not enought data
	if ((r = ttls_fetch_input(tls, 5)) != 0) {
		/* No alert on a read error. */
		TTLS_DEBUG_RET(1, "ttls_fetch_input", r);
		return r;
	}

	buf = tls->in_hdr;

	TTLS_DEBUG_BUF(4, "record header", buf, ttls_hdr_len(tls));
	 ...
	 *	 3  .   4   message length
	 */
	TTLS_DEBUG_MSG(3, ("client hello v3, message type: %d",
				   buf[0]));
	...
	TTLS_DEBUG_MSG(3, ("client hello v3, message len.: %d",
				   (tls->in_len[0] << 8) | tls->in_len[1]));

	TTLS_DEBUG_MSG(3, ("client hello v3, protocol version: [%d:%d]",
				   buf[1], buf[2]));

	ttls_read_version(&major, &minor, tls->conf->transport, buf + 1);
...
#if defined(TTLS_DTLS_ANTI_REPLAY)
		if (ttls_dtls_replay_check(tls) != 0)
		{
			TTLS_DEBUG_MSG(1, ("replayed record, discarding"));
	...
	if (msg_len > TTLS_MAX_CONTENT_LEN)
	{
		TTLS_DEBUG_MSG(1, ("bad client hello message"));
		return(TTLS_ERR_BAD_HS_CLIENT_HELLO);
	}
	...
	// TODO AK: just use ingress buffer as in ttls_read_record_layer().
	if ((r = ttls_fetch_input(tls, ttls_hdr_len(tls) + msg_len)) != 0)
	{
		TTLS_DEBUG_RET(1, "ttls_fetch_input", r);
		return r;
	}
	...
	TTLS_DEBUG_BUF(4, "record contents", buf, msg_len);

	tls->hs->update_checksum(tls, buf, msg_len);
	 ...
	 */
	if (msg_len < ttls_hs_hdr_len(tls))
	{
		TTLS_DEBUG_MSG(1, ("bad client hello message"));
		return(TTLS_ERR_BAD_HS_CLIENT_HELLO);
	}

	TTLS_DEBUG_MSG(3, ("client hello v3, handshake type: %d", buf[0]));

	if (buf[0] != TTLS_HS_CLIENT_HELLO)
	{
		TTLS_DEBUG_MSG(1, ("bad client hello message"));
		return(TTLS_ERR_BAD_HS_CLIENT_HELLO);
	}

	TTLS_DEBUG_MSG(3, ("client hello v3, handshake len.: %d",
				   (buf[1] << 16) | (buf[2] << 8) | buf[3]));

	/* We don't support fragmentation of ClientHello (yet?) */
	if (buf[1] != 0 ||
		msg_len != ttls_hs_hdr_len(tls) + ((buf[2] << 8) | buf[3]))
	{
		TTLS_DEBUG_MSG(1, ("bad client hello message"));
		return(TTLS_ERR_BAD_HS_CLIENT_HELLO);
	}
	...
	if (tls->conf->transport == TTLS_TRANSPORT_DATAGRAM)
	{
		...
		tls->hs->out_msg_seq = cli_msg_seq;
		tls->hs->in_msg_seq  = cli_msg_seq + 1;
		...
		{
			TTLS_DEBUG_MSG(1, ("ClientHello fragmentation not supported"));
			return(TTLS_ERR_FEATURE_UNAVAILABLE);
		}
	}
#endif /* TTLS_PROTO_DTLS */

	buf += ttls_hs_hdr_len(tls);
	msg_len -= ttls_hs_hdr_len(tls);
	 ...
	 */
	TTLS_DEBUG_BUF(3, "client hello, version", buf, 2);

	ttls_read_version(&tls->major_ver, &tls->minor_ver,
					  tls->conf->transport, buf);

	tls->hs->max_major_ver = tls->major_ver;
	tls->hs->max_minor_ver = tls->minor_ver;

	if (tls->major_ver < tls->conf->min_major_ver ||
		tls->minor_ver < tls->conf->min_minor_ver)
	{
		TTLS_DEBUG_MSG(1, ("client only supports tls smaller than minimum"
					" [%d:%d] < [%d:%d]",
	 ...
	 */
	TTLS_DEBUG_BUF(3, "client hello, random bytes", buf + 2, 32);

	memcpy(tls->hs->randbytes, buf + 2, 32);
	...
	TTLS_DEBUG_BUF(3, "client hello, session id", buf + 35, sess_len);
	...
	if (tls->conf->transport == TTLS_TRANSPORT_DATAGRAM)
	{
		...
		if (tls->conf->f_cookie_check != NULL) {
			...
			{
				TTLS_DEBUG_MSG(2, ("cookie verification failed"));
				tls->hs->verify_cookie_len = 1;
			}
			else
			{
				TTLS_DEBUG_MSG(2, ("cookie verification passed"));
				tls->hs->verify_cookie_len = 0;
			}
		}
		else
#endif /* TTLS_DTLS_HELLO_VERIFY */
		{
			...
			TTLS_DEBUG_MSG(2, ("cookie verification skipped"));
		}
	...
	TTLS_DEBUG_BUF(3, "client hello, ciphersuitelist",
	...
	TTLS_DEBUG_BUF(3, "client hello, compression",
					  buf + comp_offset + 1, comp_len);

	tls->session_negotiate->compression = TTLS_COMPRESS_NULL;
	...
	TTLS_DEBUG_BUF(3, "client hello extensions", ext, ext_len);
	...
	while (ext_len != 0)
	{
		...
		case TTLS_TLS_EXT_SERVERNAME:
			TTLS_DEBUG_MSG(3, ("found ServerName extension"));
		...
		case TTLS_TLS_EXT_SUPPORTED_POINT_FORMATS:
			TTLS_DEBUG_MSG(3, ("found supported point formats extension"));
			tls->hs->cli_exts |= TTLS_TLS_EXT_SUPPORTED_POINT_FORMATS_PRESENT;
		...
		case TTLS_TLS_EXT_MAX_FRAGMENT_LENGTH:
			TTLS_DEBUG_MSG(3, ("found max fragment length extension"));
		...
		case TTLS_TLS_EXT_ENCRYPT_THEN_MAC:
			TTLS_DEBUG_MSG(3, ("found encrypt then mac extension"));
		...
		case TTLS_TLS_EXT_EXTENDED_MASTER_SECRET:
			TTLS_DEBUG_MSG(3, ("found extended master secret extension"));
		...
		case TTLS_TLS_EXT_SESSION_TICKET:
			TTLS_DEBUG_MSG(3, ("found session ticket extension"));
	...
	{
		if (p[0] == (unsigned char)((TTLS_FALLBACK_SCSV_VALUE >> 8) & 0xff) &&
			p[1] == (unsigned char)((TTLS_FALLBACK_SCSV_VALUE	 ) & 0xff))
		{
			TTLS_DEBUG_MSG(2, ("received FALLBACK_SCSV"));

			if (tls->minor_ver < tls->conf->max_minor_ver)
			{
				TTLS_DEBUG_MSG(1, ("inapropriate fallback"));
	...
	if (sig_hash_alg_ext_present == 0)
	{
		...
		if (ttls_check_sig_hash(tls, md_default) != 0)
	...
	for (i = 0, p = buf + ciph_offset + 2; i < ciph_len; i += 2, p += 2)
	{
		if (p[0] == 0 && p[1] == TTLS_EMPTY_RENEGOTIATION_INFO)
		{
			TTLS_DEBUG_MSG(3, ("received TLS_EMPTY_RENEGOTIATION_INFO "));
			tls->secure_renegotiation = TTLS_SECURE_RENEGOTIATION;
			break;
		}
	}
	 ...
	 */
	if (tls->secure_renegotiation != TTLS_SECURE_RENEGOTIATION &&
		tls->conf->allow_legacy_renegotiation == TTLS_LEGACY_BREAK_HANDSHAKE)
	{
...
#if defined(TTLS_SRV_RESPECT_CLIENT_PREFERENCE)
	...
	if (got_common_suite)
	{
		TTLS_DEBUG_MSG(1, ("got ciphersuites in common, "
	...
	{
		TTLS_DEBUG_MSG(1, ("got no ciphersuites in common"));
		ttls_send_alert_message(tls, TTLS_ALERT_LEVEL_FATAL,
						TTLS_ALERT_MSG_HANDSHAKE_FAILURE);
...
have_ciphersuite:
	TTLS_DEBUG_MSG(2, ("selected ciphersuite: %s", ciphersuite_info->name));
	...
	if (tls->minor_ver == TTLS_MINOR_VERSION_3)
	{
		...
		{
			ttls_md_type_t md_alg = ttls_sig_hash_set_find(&tls->hs->hash_algs,
										  sig_alg);
			TTLS_DEBUG_MSG(3, ("client hello v3, signature_algorithm ext: %d",
						ttls_hash_from_md_alg(md_alg)));
		}
		else
		{
			TTLS_DEBUG_MSG(3, ("no hash algorithm for signature algorithm "
						"%d - should not happen", sig_alg));
		}
	}
#endif

	TTLS_DEBUG_MSG(2, ("<= parse client hello"));

	return 0;
}

static void ssl_write_encrypt_then_mac_ext(ttls_context *tls,
					unsigned char *buf,
					size_t *olen)
{
	unsigned char *p = buf;
	const ttls_ciphersuite_t *suite = NULL;
	 ...
	 */
	if ((suite = ttls_ciphersuite_from_id(
	...
	TTLS_DEBUG_MSG(3, ("server hello, adding encrypt then mac extension"));
...
#if defined(TTLS_EXTENDED_MASTER_SECRET)
static void ssl_write_extended_ms_ext(ttls_context *tls,
				   unsigned char *buf,
				   size_t *olen)
{
	unsigned char *p = buf;

	if (tls->hs->extended_ms == TTLS_EXTENDED_MS_DISABLED ||
		tls->minor_ver == TTLS_MINOR_VERSION_0)
	{
		*olen = 0;
		return;
	...
	TTLS_DEBUG_MSG(3, ("server hello, adding extended master secret "
						"extension"));
...
#if defined(TTLS_SESSION_TICKETS)
static void ssl_write_session_ticket_ext(ttls_context *tls,
					  unsigned char *buf,
					  size_t *olen)
{
	unsigned char *p = buf;

	if (tls->hs->new_session_ticket == 0)
	{
		*olen = 0;
		return;
	}

	TTLS_DEBUG_MSG(3, ("server hello, adding session ticket extension"));
...
static void ssl_write_renegotiation_ext(ttls_context *tls,
					 unsigned char *buf,
					 size_t *olen)
{
	unsigned char *p = buf;
	...
	TTLS_DEBUG_MSG(3, ("server hello, secure renegotiation extension"));
...
#if defined(TTLS_MAX_FRAGMENT_LENGTH)
static void ssl_write_max_fragment_length_ext(ttls_context *tls,
					   unsigned char *buf,
					   size_t *olen)
{
	unsigned char *p = buf;

	if (tls->session_negotiate->mfl_code == TTLS_MAX_FRAG_LEN_NONE)
	{
		*olen = 0;
		return;
	}

	TTLS_DEBUG_MSG(3, ("server hello, max_fragment_length extension"));
						   ...
						   size_t *olen)
{
	...
	TTLS_DEBUG_MSG(3, ("server hello, supported_point_formats extension"));
				...
				unsigned char *buf, size_t *olen)
{
	...
	TTLS_DEBUG_MSG(3, ("server hello, adding alpn extension"));
...
static int ssl_write_server_hello(ttls_context *tls)
{
	time_t t;
	int r;
	...
	TTLS_DEBUG_MSG(2, ("=> write server hello"));

#if defined(TTLS_DTLS_HELLO_VERIFY)
	if (tls->conf->transport == TTLS_TRANSPORT_DATAGRAM &&
		tls->hs->verify_cookie_len != 0)
	{
		TTLS_DEBUG_MSG(2, ("client hello was not authenticated"));
		TTLS_DEBUG_MSG(2, ("<= write server hello"));
	...
	TTLS_DEBUG_MSG(3, ("server hello, chosen version: [%d:%d]",
						buf[4], buf[5]));
	...
	*p++ = (unsigned char)(t	  );
	...
	TTLS_DEBUG_MSG(3, ("server hello, current time: %lu", t));
	...
	TTLS_DEBUG_BUF(3, "server hello, random bytes", buf + 6, 32);
	...
	{
		TTLS_DEBUG_MSG(3, ("session successfully restored from cache"));
		tls->hs->resume = 1;
	}
	...
	else
	{
		...
		if ((r = ttls_derive_keys(tls)) != 0)
		{
			TTLS_DEBUG_RET(1, "ttls_derive_keys", r);
			return r;
		}
	}
	...
	*p++ = (unsigned char) tls->session_negotiate->id_len;
	...
	TTLS_DEBUG_BUF(3,   "server hello, session id", buf + 39, n);
	...
	*p++ = (unsigned char)(tls->session_negotiate->compression	 );
	...
	TTLS_DEBUG_MSG(3, ("server hello, chosen ciphersuite: %s",
		   ttls_get_ciphersuite_name(tls->session_negotiate->ciphersuite)));
	TTLS_DEBUG_MSG(3, ("server hello, compress alg.: 0x%02X",
	...
	TTLS_DEBUG_MSG(3, ("server hello, total extension length: %d", ext_len));
	...
	tls->out_msg[0]  = TTLS_HS_SERVER_HELLO;

	r = ttls_write_record(tls);
...
static int ssl_write_certificate_request(ttls_context *tls)
{
	...
	unsigned char *buf, *p;
	const unsigned char * const end = tls->out_msg + TTLS_MAX_CONTENT_LEN;
	...
	if (tls->minor_ver == TTLS_MINOR_VERSION_3)
	{
		...
		for (cur = tls->conf->sig_hashes; *cur != TTLS_MD_NONE; cur++)
		{
			...
			if (TTLS_HASH_NONE == hash || ttls_set_calc_verify_md(tls, hash))
				continue;
	...
	if (tls->conf->cert_req_ca_list ==  TTLS_CERT_REQ_CA_LIST_ENABLED)
	{
		...
		while (crt != NULL && crt->version != 0)
		{
			...
			{
				TTLS_DEBUG_MSG(1, ("skipping CAs: buffer too short"));
				break;
			}
			...
			*p++ = (unsigned char)(dn_size	 );
			...
			TTLS_DEBUG_BUF(3, "requested DN", p - dn_size, dn_size);
	...
	tls->out_msglen  = p - buf;
	tls->out_msgtype = TTLS_MSG_HANDSHAKE;
	tls->out_msg[0]  = TTLS_HS_CERTIFICATE_REQUEST;
...
static int ssl_get_ecdh_params_from_cert(ttls_context *tls)
{
	...
	if ((r = ttls_ecdh_get_params(&tls->hs->ecdh_ctx,
					 ...
					 TTLS_ECDH_OURS)) != 0)
	{
		TTLS_DEBUG_RET(1, ("ttls_ecdh_get_params"), r);
		return r;
	}
...
static int ssl_write_server_key_exchange(ttls_context *tls)
{
	...
	if (ttls_ciphersuite_uses_dhe(ciphersuite_info))
	{
		...
		{
			TTLS_DEBUG_MSG(1, ("no DH parameters set"));
		 ...
		 * } ServerDHParams;
		 */
		if ((r = ttls_dhm_set_group(&tls->hs->dhm_ctx,
						   &tls->conf->dhm_P,
						   &tls->conf->dhm_G)) != 0)
		{
			TTLS_DEBUG_RET(1, "ttls_dhm_set_group", r);
			return r;
		}

		if ((r = ttls_dhm_make_params(&tls->hs->dhm_ctx,
						(int) ttls_mpi_size(&tls->hs->dhm_ctx.P),
	...
	if (ttls_ciphersuite_uses_ecdhe(ciphersuite_info))
	{
		...
		for (gid = tls->conf->curve_list; *gid != TTLS_ECP_DP_NONE; gid++)
			for (curve = tls->hs->curves; *curve != NULL; curve++)
				if ((*curve)->grp_id == *gid)
...
curve_matching_done:
		...
		{
			TTLS_DEBUG_MSG(1, ("no matching curve for ECDHE"));
			return(TTLS_ERR_NO_CIPHER_CHOSEN);
		}

		TTLS_DEBUG_MSG(2, ("ECDHE curve: %s", (*curve)->name));

		if ((r = ttls_ecp_group_load(&tls->hs->ecdh_ctx.grp,
						   (*curve)->grp_id)) != 0)
		{
			TTLS_DEBUG_RET(1, "ttls_ecp_group_load", r);
			return r;
		}

		if ((r = ttls_ecdh_make_params(&tls->hs->ecdh_ctx, &len,
						  p, TTLS_MAX_CONTENT_LEN - n,
						  tls->conf->f_rng, tls->conf->p_rng)) != 0)
		{
			TTLS_DEBUG_RET(1, "ttls_ecdh_make_params", r);
			return r;
		}
	...
	if (ttls_ciphersuite_uses_server_signature(ciphersuite_info))
	{
		...
		TTLS_DEBUG_MSG(3, ("pick hash algorithm %d for signing", md_alg));
		 ...
		 */
		if (ttls_own_key(tls) == NULL)
		{
			TTLS_DEBUG_MSG(1, ("got no private key"));
		...
		if ((r = ttls_pk_sign(ttls_own_key(tls), md_alg, hash, hashlen,
						p + 2 , &signature_len, tls->conf->f_rng, tls->conf->p_rng)) != 0)
		{
			TTLS_DEBUG_RET(1, "ttls_pk_sign", r);
			return r;
		}
		...
		*(p++) = (unsigned char)(signature_len	 );
		...
		TTLS_DEBUG_BUF(3, "my signature", p, signature_len);
	...
	tls->out_msglen  = 4 + n;
	tls->out_msgtype = TTLS_MSG_HANDSHAKE;
	tls->out_msg[0]  = TTLS_HS_SERVER_KEY_EXCHANGE;
...
static int ssl_write_server_hello_done(ttls_context *tls)
{
	int r;

	TTLS_DEBUG_MSG(2, ("=> write server hello done"));

	tls->out_msglen  = 4;
	tls->out_msgtype = TTLS_MSG_HANDSHAKE;
	tls->out_msg[0]  = TTLS_HS_SERVER_HELLO_DONE;
					   ...
					   const unsigned char *end)
{
	...
	*p += 2;
	...
	if ((r = ttls_dhm_read_public(&tls->hs->dhm_ctx, *p, n)) != 0)
	{
		TTLS_DEBUG_RET(1, "ttls_dhm_read_public", r);
				...
				size_t pms_offset)
{
	...
	unsigned char *pms = tls->hs->premaster + pms_offset;
	unsigned char ver[2];
	...
	{
		TTLS_DEBUG_MSG(1, ("got no RSA private key"));
		return(TTLS_ERR_PRIVATE_KEY_REQUIRED);
	}
	 ...
	 */
	if (tls->minor_ver != TTLS_MINOR_VERSION_0)
	{
		if (*p++ != ((len >> 8) & 0xFF) ||
			*p++ != ((len	 ) & 0xFF))
		{
	...
	ttls_write_version(tls->hs->max_major_ver,
					   tls->hs->max_minor_ver,
	...
	r = ttls_pk_decrypt(ttls_own_key(tls), p, len,
...
#if defined(TTLS_DEBUG_ALL)
	if (diff != 0)
		TTLS_DEBUG_MSG(1, ("bad client key exchange message"));
#endif

	if (sizeof(tls->hs->premaster) < pms_offset ||
		sizeof(tls->hs->premaster) - pms_offset < 48)
	{
		TTLS_DEBUG_MSG(1, ("should never happen"));
		return(TTLS_ERR_INTERNAL_ERROR);
	}
...
static int ssl_parse_client_key_exchange(ttls_context *tls)
{
	...
	TTLS_DEBUG_MSG(2, ("=> parse client key exchange"));
	...
	p = tls->in_msg + ttls_hs_hdr_len(tls);
	...
	if (tls->in_msg[0] != TTLS_HS_CLIENT_KEY_EXCHANGE)
	{
		TTLS_DEBUG_MSG(1, ("bad client key exchange message"));
		return(TTLS_ERR_BAD_HS_CLIENT_KEY_EXCHANGE);
	}
		...
		ciphersuite_info->key_exchange == TTLS_KEY_EXCHANGE_ECDH_ECDSA)
	{
		...
		if ((r = ttls_ecdh_calc_secret(&tls->hs->ecdh_ctx,
						  &tls->hs->pmslen,
						   tls->hs->premaster,
						   TTLS_MPI_MAX_SIZE,
						   tls->conf->f_rng, tls->conf->p_rng)) != 0)
...
static int ssl_parse_certificate_verify(ttls_context *tls)
{
	int r = TTLS_ERR_FEATURE_UNAVAILABLE;
	size_t i, sig_len;
	...
	TTLS_DEBUG_MSG(2, ("=> parse certificate verify"));
	...
	{
		TTLS_DEBUG_MSG(2, ("<= skip parse certificate verify"));
		tls->state++;
		return 0;
	}
	...
	do {
		...
		{
			TTLS_DEBUG_RET(1, ("ttls_read_record_layer"), r);
			return r;
		}
	...
	if (0 != r)
	{
		TTLS_DEBUG_RET(1, ("ttls_handle_message_type"), r);
		return r;
	}
	...
	if (tls->in_msgtype != TTLS_MSG_HANDSHAKE ||
		tls->in_msg[0] != TTLS_HS_CERTIFICATE_VERIFY)
	{
		TTLS_DEBUG_MSG(1, ("bad certificate verify message"));
	...
	if (tls->minor_ver == TTLS_MINOR_VERSION_3)
	{
		 ...
		 * Hash
		 */
		md_alg = ttls_md_alg_from_hash(tls->in_msg[i]);

		if (md_alg == TTLS_MD_NONE || ttls_set_calc_verify_md(tls, tls->in_msg[i]))
		{
			TTLS_DEBUG_MSG(1, ("peer not adhering to requested sig_alg"
								" for verify message"));
			return(TTLS_ERR_BAD_HS_CERTIFICATE_VERIFY);
		}
		 ...
		 * Signature
		 */
		if ((pk_alg = ttls_pk_alg_from_sig(tls->in_msg[i]))
						== TTLS_PK_NONE)
		{
			TTLS_DEBUG_MSG(1, ("peer not adhering to requested sig_alg"
								" for verify message"));
		...
		{
			TTLS_DEBUG_MSG(1, ("sig_alg doesn't match cert key"));
			return(TTLS_ERR_BAD_HS_CERTIFICATE_VERIFY);
		}
	...
	{
		TTLS_DEBUG_RET(1, "ttls_pk_verify", r);
		return r;
	}
...
#if defined(TTLS_SESSION_TICKETS)
static int ssl_write_new_session_ticket(ttls_context *tls)
{
	int r;
	size_t tlen;
	uint32_t lifetime;

	TTLS_DEBUG_MSG(2, ("=> write new session ticket"));

	tls->out_msgtype = TTLS_MSG_HANDSHAKE;
	tls->out_msg[0]  = TTLS_HS_NEW_SESSION_TICKET;
	...
	if ((r = tls->conf->f_ticket_write(tls->conf->p_ticket,
					...
					tls->out_msg + 10,
					tls->out_msg + TTLS_MAX_CONTENT_LEN,
					&tlen, &lifetime)) != 0)
	{
		TTLS_DEBUG_RET(1, "ttls_ticket_write", r);
		tlen = 0;
	}
 ...
 */
int
ttls_handshake_server_step(TlsCtx *tls)
{
	int r = 0;

	if (tls->state == TTLS_HANDSHAKE_OVER || tls->hs == NULL)
		return(TTLS_ERR_BAD_INPUT_DATA);

	TTLS_DEBUG_MSG(2, ("server state: %d", tls->state));
	...
	switch(tls->state) {
		...
		case TTLS_CLIENT_CHANGE_CIPHER_SPEC:
			r = ttls_parse_change_cipher_spec(tls);
			break;

		case TTLS_CLIENT_FINISHED:
			r = ttls_parse_finished(tls);
			break;
...
#endif
				r = ttls_write_change_cipher_spec(tls);
			break;

		case TTLS_SERVER_FINISHED:
			r = ttls_write_finished(tls);
			break;
		...
		default:
			TTLS_DEBUG_MSG(1, ("invalid state %d", tls->state));
			return(TTLS_ERR_BAD_INPUT_DATA);
	}
