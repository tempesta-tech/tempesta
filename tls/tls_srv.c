...
ttls_parse_alpn_ext(TlsCtx *tls, const unsigned char *buf, size_t len)
{
	...
	for (theirs = start; theirs != end; theirs += cur_len) {
		...
		if (!cur_len) {
			...
			ttls_send_alert(tls, TTLS_ALERT_LEVEL_FATAL,
					TTLS_ALERT_MSG_ILLEGAL_PARAMETER, 0);
			return -EBADMSG;
		}
	}
	...
	TTLS_WARN(tls, "ClientHello: cannot find matching ALPN for %.*s%s\n",
		  ...
		  *start + 1 < end - start
			...
			: "");
	ttls_send_alert(tls, TTLS_ALERT_LEVEL_FATAL,
			TTLS_ALERT_MSG_NO_APPLICATION_PROTOCOL, 0);
	return -EBADMSG;
}
...
ttls_choose_ciphersuite(TlsCtx *tls)
{
	...
	if (got_common_suite) {
		...
		ttls_send_alert(tls, TTLS_ALERT_LEVEL_FATAL,
				TTLS_ALERT_MSG_HANDSHAKE_FAILURE, 0);
		return -EINVAL;
	} else {
...
bad_version:
		...
		ttls_send_alert(tls, TTLS_ALERT_LEVEL_FATAL,
				TTLS_ALERT_MSG_PROTOCOL_VERSION, 0);
		return -EINVAL;
	}
				...
				struct page **pg)
{
	...
	*pg = NULL;
	...
	return __ttls_send_record(tls, sgt, 0);
}
