...
typedef struct TlsX509Crt {
	struct {
		...
		unsigned int		n_pgs;
		const unsigned char	*pages[TTLS_CERT_RAW_P_N];
	}			raw;
	ttls_x509_buf		tbs;
...
#define FOR_EACH_X509_CRT_RAW_PG(crt, frag_p, frag_sz, lambda)		\
do {									\
	int i;								\
	for (i = 0; i < (crt)->raw.n_pgs; ++i) {			\
		size_t frag_sz = i + 1 < (crt)->raw.n_pgs		\
				 ? PAGE_SIZE				\
				 : (crt)->raw.tot_len - i * PAGE_SIZE;	\
		const void *frag_p = (crt)->raw.pages[i];		\
		lambda;							\
	}								\
} while (0)
