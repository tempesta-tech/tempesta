...
public:
	...
	fs::path get_log_path() const { return log_path_; }
	fs::path get_pid_file() const { return pid_file_; }
...
private:
	Mode mode_{Mode::DAEMON};				// Default to daemon mode
	fs::path log_path_{"/var/log/tempesta/tfw_logger.log"}; // Log file path
	fs::path pid_file_{"/var/run/tfw_logger.pid"};		// PID file path
	size_t buffer_size_{4 * 1024 * 1024};			// 4MB default buffer size
	size_t cpu_count_{0};					// 0 means auto-detect
	ClickHouseConfig clickhouse_;				// ClickHouse configuration
