...
static int
add_backend_entry(const TfwAddr *addr)
{
	TfwBackendSockEntry *entry;

	entry = kzalloc(sizeof(*entry), GFP_KERNEL);
	entry->addr = addr;
	list_add_tail(&entry->list, &backend_socks);
...
static TfwCfgSpec sock_backend_cfg_spec[] = {
	{
		"backends",
		"backends 127.0.0.1:8080;",
		"A list of back-end HTTP servers.",

		.val_each = true,
		.call_addr = add_backend_entry
	},
	{}
};
...
TfwCfgMod tfw_mod_sock_backend = {
	.name = "backends",
	.cfg_spec_arr = sock_backend_cfg_spec,
