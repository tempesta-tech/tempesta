...
def start(unlim, resp):
	be_port = conf.get_beport()
	httpd = Server(('127.0.0.1', be_port), BackendHTTPRequestHandler)
	...
	if pid == 0:
		...
		if pid == 0:
			...
			os._exit(0)
	"""Wait while the child writes his pid. We can't wait it to 
	the end (serve forever). So let it be a fraction of a second.
	"""
	time.sleep(0.2)
	rd = open("be.pid", 'a+')
	r_pid = rd.read()
