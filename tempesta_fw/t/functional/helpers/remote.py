...
import paramiko, re, threading, os, abc
...
class Node:
    ...
    @abc.abstractmethod
    def mkdir(self, dir): pass

    @abc.abstractmethod
    def copy_file(self, filename, content, dir = None): pass
...
class LocalNode(Node):
    def __init__(self, hostname, workdir):
        super(LocalNode, self).__init__(hostname, workdir)
...
class RemoteNode(Node):
    def __init__(self,  hostname, workdir, user, port = 22):
        super(RemoteNode, self).__init__(hostname, workdir)
        self.user = user
        self.port = port
        self.connect()
