...
TEST(tfw_str_hash, calcs_diff_hash_for_diff_str)
{
...
/* This is expected behavior because of optimizations in the hash function. */
TEST(tfw_str_hash, calcs_diff_hash_for_diff_chunks_n)
{
...
TEST(tfw_str_hash, hashes_all_chars)
{
/* FIXME: this test fails. */
	...
	for (i = 0; i < 255; ++i) {
		...
		h2 = tfw_str_hash(&s2);
		if (h1 == h2)
			TEST_FAIL("Equal hashes for different strings:\n"
			          " s1: %.*s\n s2: %.*s",
				  s1.len, (char *)s1.ptr,
				  s2.len, (char *)s2.ptr);
		...
		if (h1 != h2)
			TEST_FAIL("Different hashes for equal strings:\n"
			          " s1: %#08lx: %.*s\n s2: %#08lx: %.*s",
				  h1, s1.len, (char *)s1.ptr,
				  h2, s2.len, (char *)s2.ptr);
	}
#endif
}

TEST(tfw_str_hash, doesnt_read_behind_end_of_buf)
{
	char buf[256] = { 0 };
	TfwStr s = {
		.len = 0,
...
TEST_SUITE(tfw_str)
{
	...
	TEST_RUN(tfw_str_hash, calcs_diff_hash_for_diff_str);
	TEST_RUN(tfw_str_hash, calcs_diff_hash_for_diff_chunks_n);
	TEST_RUN(tfw_str_hash, hashes_all_chars);
	TEST_RUN(tfw_str_hash, doesnt_read_behind_end_of_buf);
}
