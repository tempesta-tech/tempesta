...
ss_do_send(struct sock *sk, struct sk_buff **skb_head, int flags)
{
	 ...
	 *   stream, make frames for this stream and push them to socket
	 *   write queue.
	 * - socket flag is not set, this means that we push skb to the
	 *   directly to the socket write queue in `connection_do_send`
	 *   callback and so we call `__tcp_push_pending_frames` and don't
	 *   run scheduler.
...
{
	/*
	 * We can't send TCP shutdown if there is some
	 * data in our scheduler. We send it later, when
