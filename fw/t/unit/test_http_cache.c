...
TEST(http_cache, health_monitor)
{
	...
	TfwStr host_header = { .data = (void *)"any.host.com", .len = 12 };
	test_req->host = host_header;

	TfwStr uri_path = { .data = (void *)"/health", .len = 7 };
	test_req->uri_path = uri_path;
...
/**
 * Test cache key calculation with an empty vhost name.
 * An empty vhost name should still result in a deterministic key,
 * different from a key with a non-empty vhost name.
 */
TEST(http_cache, empty_vhost_name)
{
	unsigned long key_empty_vhost, key_named_vhost, key_empty_vhost_again;
	TfwVhost vhost_empty;
	BasicStr empty_name = { .data = "", .len = 0 };
	...
	/* Calculate key with empty vhost name */
	test_req->vhost = &vhost_empty;
	test_req->hash = 0;
	key_empty_vhost = tfw_http_req_key_calc(test_req);

	/* Calculate key with a named vhost */
	test_req->vhost = &test_vhost1;
	test_req->hash = 0;
	key_named_vhost = tfw_http_req_key_calc(test_req);

	/* The keys should be different */
	EXPECT_NE(key_empty_vhost, key_named_vhost);

	/* Calculate key with empty vhost name again to ensure consistency */
	test_req->vhost = &vhost_empty;
	test_req->hash = 0;
	key_empty_vhost_again = tfw_http_req_key_calc(test_req);
	EXPECT_EQ(key_empty_vhost, key_empty_vhost_again);
}
