...
void
tfw_http_req_skb_destructor(struct sk_buff *skb)
{
	TfwHttpReq *req = (TfwHttpReq *)TFW_SKB_CB(skb)->opaque_data;

	/*
	 * We can safely access `req` pointer here, because
	 * we remove change this destructor to default in
	 * `tfw_http_req_on_tcp_entail`.
	 * If server connection will be dropped before `on_tcp_entail`
	 * callback is called, this destructor will be called during
	 * server connection release.
	 * request never destroed on the client side until the
	 * response for current request will be fully received.
	 */
	TFW_SKB_CB(skb)->opaque_data = CLIENT_MEM_FROM_CONN(req->conn);
...
static int
tfw_http_req_on_tcp_entail(void *conn, struct sk_buff *skb)
{
	TfwSrvConn *srv_conn = (TfwSrvConn *)conn;
	TfwHttpReq *req = (TfwHttpReq *)TFW_SKB_CB(skb)->opaque_data;

	/*
	 * Should be called at the beginning of this function,
	 * before request deleting to prevent memory corruption
	 * from `tfw_http_req_skb_destructor` when we will free
	 * skbs after this callback fails.
	 */
	__ss_skb_set_owner(skb, ss_skb_dflt_destructor,
			   CLIENT_MEM_FROM_CONN(req->conn));
...
tfw_http_req_init_for_send(TfwSrvConn *srv_conn, TfwHttpReq *req)
{
	...
	if (likely(test_bit(TFW_HTTP_B_HMONITOR, req->flags)))
		return;

	TFW_SKB_CB(req->msg.skb_head)->on_tcp_entail =
		tfw_http_req_on_tcp_entail;
	__ss_skb_set_owner(req->msg.skb_head, tfw_http_req_skb_destructor,
			   req);
}
