 ...
 */
static inline void
tfw_http_req_enlist(TfwSrvConn *srv_conn, TfwHttpReq *req, bool save_conn)
{
		 ...
		 bool resched, bool save_conn)
{
	...
	tfw_http_req_enlist(srv_conn, req, save_conn);
	/*
...
tfw_http_req_resched(TfwHttpReq *req, TfwServer *srv, struct list_head *eq)
{
	...
	ret = tfw_http_req_fwd(sch_conn, req, eq, true, save_conn);
	/*
...
tfw_http_fwdq_resched(TfwSrvConn *srv_conn, struct list_head *resch_queue,
		      struct list_head *eq, bool srv_conn_deleted)
{
	TfwHttpReq *req, *tmp;
	TfwServer *srv = (TfwServer *)srv_conn->peer;
 ...
 */
static void
tfw_http_conn_shrink_fwdq_resched(TfwSrvConn *srv_conn, bool srv_conn_deleted)
{
	LIST_HEAD(eq);
	LIST_HEAD(schq);
	...
	tfw_http_fwdq_resched(srv_conn, &schq, &eq, srv_conn_deleted);
...
tfw_http_cli_conn_drop(TfwCliConn *cli_conn)
{
	...
	list_for_each_entry_safe(req, tmp_req, seq_queue, msg.seq_list) {
		...
		if (unused) {
			...
			TfwSrvConn *fwd_conn = req->fwd_conn;
			 ...
			 * problem:
			 * - if `TFW_CONN_B_DEL` is set or `ss_active` is not active
			 * we  iterate over all requests in `fwd_queue` under the
			 * `seq_qlock`, same as this function, so we never destroy
			 * server connection, before this function will be finished.
...
next_msg:
	...
	if (unlikely(test_bit(TFW_CONN_B_NEED_RESCHED_AND_STOP,
		     &((TfwSrvConn *)conn)->flags)))
	{
		...
		/* TODO should we reschedule requests here ? */
	}
