...
do {									\
	WARN_ON_ONCE(stream->xmit.mark && stream->xmit.mark != mark);	\
	WARN_ON_ONCE(stream->xmit.tls_type &&				\
	...
	case HTTP2_RST_STREAM:
		...
		break;
	case HTTP2_GOAWAY:
		target = &h2->goaway_skb_head;
		h2->goaway_tls_type = tls_type;
		BUG_ON(flags & SS_F_CONN_CLOSE);
		break;
	default:
		break;
	}
...
#undef TFW_H2_INIT_STREAM_FOR_XMIT
}
