 ...
 *                not NULL
 * @do_send	- callback to send skb_buff;
 * @stream_id	- id of sender stream;
 * @frame_type	- type of frame;
 * @flags	- HTTP2 frame flags;
 * @tls_type	- tls type which should be set for this skb before sending
 *                to socket write queue;
 * @mark	- mark which should be set for this skb before sending to
 *                socket write queue;
 */
struct tfw_skb_cb {
	void 			*opaque_data;
	void 			(*destructor)(void *opaque_data);
	int 			(*do_send)(struct sk_buff **skb_head);		
