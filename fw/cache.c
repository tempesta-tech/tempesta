 ...
 * - for http connections - pages are reused in skbs and TFW_SKBTX_SHARED_FRAG
 * is set to avoid any data copies.
 * - for https connections - pages are reused in skbs and TFW_SKBTX_SHARED_FRAG
 * is set, but in-place crypto operations are not allowed, so data copy happens
 * right before data is pushed into network.
 * - for h2 connections - every response has unique frame header, so need to
 * copy on constructing response body from cache. TFW_SKBTX_SHARED_FRAG is left
			  ...
			  unsigned long body_sz, bool h2)
{
	...
	if (!it->skb || (it->frag + 1 >= MAX_SKB_FRAGS)
	    || (sh_frag == !(skb_shinfo(it->skb)->tx_flags & TFW_SKBTX_SHARED_FRAG)))
	{
