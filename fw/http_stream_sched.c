 ...
 * (streams 3 and 5) and choose the stream (which is active) with the lowest
 * weight.
 *
...
{
	TfwH2Ctx __maybe_unused *ctx = container_of(sched, TfwH2Ctx, sched);
	struct eb64_node *node = eb64_first(&sched->root.active);
	...
	while (node) {
		...
		if (tfw_h2_stream_is_active(stream)) {
			...
			node = eb64_first(&stream->sched.active);
		} else {
			/*
