 ...
 */
void
tfw_h2_stream_sched_init_entry_storage(TfwStreamSched *sched, void *base,
				       unsigned int count)
{
	TfwStreamSchedEntry *p;
	unsigned int i;

	for (i = 0, p = (TfwStreamSchedEntry *)base; i < count; i++, p++) {
		p->owner = NULL;
		INIT_LIST_HEAD(&p->in_empty_list);
		list_add_tail(&p->in_empty_list, &sched->empty_scheds);
	}
}