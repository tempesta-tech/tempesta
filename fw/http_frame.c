...
do {									\
	...
	T_FSM_STATE(HTTP2_SEND_FRAMES) {
		if (stream->xmit.frame_length) {
			...
			while ((skb = ss_skb_dequeue(&stream->xmit.skb_head))) {
				ss_skb_entail(sk, skb, stream->xmit.mark,
					      stream->xmit.tls_type);
			}
		}
		   ...
		   bool *data_is_available)
{
	...
	if (!(*data_is_available) && ctx->goaway_skb_head) {
		TfwH2Conn *conn = container_of(ctx, TfwH2Conn, h2);
		struct sock *sk = ((TfwConn *)conn)->sk;
		struct sk_buff *skb;
		unsigned int mark;

		mark = ctx->goaway_skb_head ? ctx->goaway_skb_head->mark : 0;
		while ((skb = ss_skb_dequeue(&ctx->goaway_skb_head)))
			ss_skb_entail(sk, skb, mark, ctx->goaway_tls_type);
	}

	if (!(*data_is_available) && ctx->alert_skb_head) {
		TfwH2Conn *conn = container_of(ctx, TfwH2Conn, h2);
		struct sock *sk = ((TfwConn *)conn)->sk;
		struct sk_buff *skb;
		unsigned int mark;

		mark = ctx->alert_skb_head ? ctx->alert_skb_head->mark : 0;
		while ((skb = ss_skb_dequeue(&ctx->alert_skb_head)))
			ss_skb_entail(sk, skb, mark, ctx->alert_tls_type);
	}
