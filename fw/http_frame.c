		    ...
		    TfwCloseType type)
{
	...
	TFW_SKB_CB(msg.skb_head)->frame_type = hdr->type;
	TFW_SKB_CB(msg.skb_head)->flags = hdr->flags;
...
tfw_h2_entail_stream_skb(TfwH2Ctx *ctx, TfwStream *stream, unsigned int *len)
{
	...
	unsigned int mark = TFW_SKB_CB(stream->xmit.skb_head)->mark;
	...
	if (stream->xmit.skb_head && !TFW_SKB_CB(stream->xmit.skb_head)->tls_type) {
		TFW_SKB_CB(stream->xmit.skb_head)->tls_type = tls_type;
		TFW_SKB_CB(stream->xmit.skb_head)->mark = mark;
	}
...
tfw_h2_send_final_data(TfwH2Ctx *ctx, struct sk_buff **skb_head)
{
	...
	while ((skb = ss_skb_dequeue(skb_head))) {
		if (TFW_SKB_CB(skb)->tls_type) {
			tls_type = TFW_SKB_CB(skb)->tls_type;
			mark = TFW_SKB_CB(skb)->mark;
		}
...
tfw_h2_stream_xmit_prepare_resp(TfwStream *stream)
{
	...
	mark = TFW_SKB_CB(stream->xmit.skb_head)->mark;
