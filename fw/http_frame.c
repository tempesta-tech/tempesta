 ...
 *
 * WARNING: This function relies on data sending in @tfw_h2_conn_recv_finish()
 * DON'T call this function on TX path.
 */
static int
__tfw_h2_send_frame_on_rx(TfwH2Ctx *ctx, TfwFrameHdr *hdr, TfwStr *data)
{
	int r;
	TfwMsgIter it;
	TfwMsg msg = {};
...
do {									\
	...
	case HTTP2_SETTINGS:
		...
		r = tfw_h2_settings_init_list(&ctx->received_settings,
					      hdr->length);
		if (unlikely(r)) {
...
static inline unsigned int
__total_frame_length(unsigned int frame_length)
{
...
__tfw_h2_make_headers_frame(TfwH2Ctx *ctx, TfwStream *stream)
{
	...
	stream->xmit.headers_frame_length = __total_frame_length(frame_length);
	r = tfw_h2_stream_fsm_ignore_err(ctx, stream, type, flags);
	if (unlikely(r))
		return r;
	ctx->cur_send_headers = stream;

	return tfw_h2_insert_frame_header(stream, type, frame_length, flags);
}
			 ...
			 unsigned int frame_length)
{
	...
	r = tfw_h2_insert_frame_header(stream, type, frame_length, flags);
	if (unlikely(r))
		goto undo;

	stream->xmit.bytes_to_send += __total_frame_length(frame_length);
			   ...
			   unsigned long *snd_wnd, bool *stop)
{
	...
	T_FSM_STATE(HTTP2_SEND_HEADERS_FRAME) {
		...
		if (bytes_to_send < min_to_send && !is_last_chunk &&
		    __socket_has_inflight_data(sk)) {
			/*
			 * Stop sending only if socket has unknowledged or
			 * queued data (see @__socket_has_inflight_data()).
	...
	T_FSM_STATE(HTTP2_MAKE_DATA_FRAMES) {
		...
		*snd_wnd -= __total_frame_length(frame_length);
		fallthrough;
	}
