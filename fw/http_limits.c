...
frang_acc_history_init(FrangAcc *ra, unsigned long ts)
{
	...
	tfw_client_training_adjust_conn_num(&cli->conn_max, ra->conn_curr,
					    &cli->conn_training_num,
					    tfw_training_mode_adjust_conn_num);
}
		       ...
		       TfwVhost *dvh)
{
	 ...
	 */
	if (!tfw_training_mode_defence_req_num(atomic64_read(&cli->req_curr) + 1)) {
		if (fg_cfg->ip_block)
