...
# function return type can't be on same line
		if ($perl_version_ok &&
		    $sline =~ /$Type\s*$Ident\s*$balanced_parens\s*\{?\s*$/) {
			ERROR("FUNCTION_RETURN_TYPE_NEWLINE",
			      "function return type can't be on same line with function definition\n" . $herecurr)
		}
...
# check for if-else-if conditions on a new line
		if ($sline =~ /^.\s*\}\s*else\s*/ ||
		    $sline =~ /^.\s*\}\s*else if\s*/) {
			WARN("BRACES", "`else` or `else if` should on a new line\n" . $herecurr);
		}
