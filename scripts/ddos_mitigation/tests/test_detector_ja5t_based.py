...
class TestBackgroundMonitorReleaseUsers(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_rps(self):
        ...
        assert set(users_after[0].ipv4) == set([IPv4Address("127.0.0.3"), IPv4Address("127.0.0.4")])
    ...
    async def test_errors(self):
        ...
        users_before, users_after = await detector.find_users(current_time=1751535010, interval=5)
        assert users_before == []
        assert users_after == [User(ja5t=[13], ja5h=[23], ipv4=[IPv4Address("127.0.0.4"), IPv4Address("127.0.0.3")])]
