...
distribute_queues()
{
	...
	for irq in "${irqs[@]}"; do
		if ! old_masks[$irq]=$(<"/proc/irq/$irq/smp_affinity"); then
			echo "Error: cannot read current IRQ $irq affinity"
			return
		fi
	done
	...
	for irq in "${irqs[@]}"; do
		...
		if ! echo "$mask" > "/proc/irq/$irq/smp_affinity"; then
			echo "Error: cannot bind IRQ $irq to CPU $cpu, mask=$mask"

			for changed_irq in "${changed_irqs[@]}"; do
				if ! echo "${old_masks[$changed_irq]}" > "/proc/irq/$changed_irq/smp_affinity"; then
					echo "Error: cannot restore IRQ $changed_irq affinity, mask=${old_masks[$changed_irq]}"
				fi
			done
